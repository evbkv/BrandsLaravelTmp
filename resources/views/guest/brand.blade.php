...
<div class="card border-opacity-25 mb-3">
	...
	<div class="card-body">
		...
		<p class="card-text text-muted small"><span onclick="spa_brand({{ $one_news->brand_id }});" style="cursor: pointer;">{{ $brand->name }}</span> ⋅ {{ date("d.m.Y", strtotime($one_news->created_at)) }}</p>
	</div>
</div>
